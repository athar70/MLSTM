...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    tb_callback = TensorBoard(log_dir=f"./log/{dataset_prefix}_log", write_graph=True, write_images=True)

    # Plot and log confusion matrix after training
    ...
    file_writer_cm = tf.summary.create_file_writer(f"./log/{dataset_prefix}_log")
    
    def log_confusion_matrix():
        """
        Log confusion matrix as an image to TensorBoard after each epoch.
        """
        test_pred_raw = model.predict(X_test)
        test_pred = np.argmax(test_pred_raw, axis=1)
        cm = sklearn.metrics.confusion_matrix(np.argmax(y_test, axis=1), test_pred)

        #class_names = ['HH', 'LL', 'HL', 'LH']
    ...
    cm_callback = LambdaCallback(on_train_end=log_confusion_matrix)
    ####
